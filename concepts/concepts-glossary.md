...
| 48 | Metasploit Framework | Offensive Security | Modular exploitation framework for penetration testing with exploits, payloads, auxiliary modules, and post modules. | Day 11 |
| 49 | Meterpreter | Offensive Security | An advanced Metasploit payload that provides an interactive shell, file operations, and post-exploitation capabilities on a compromised host. | Day 11 |
| 50 | Auxiliary Module | Offensive Security | A Metasploit module used for scanning, enumeration, brute forcing, and other non-exploit actions. | Day 11 |
| 51 | Post Module | Offensive Security | A Metasploit module that runs follow-on tasks inside an active session, such as credential harvesting, privilege escalation, or pivoting. | Day 11 |
| 52 | msfvenom | Offensive Security | A Metasploit tool for generating standalone payload files and encoding shellcode for delivery outside the framework. | Day 11 |
| 47 | Phishing-Resistant Authentication (FIDO2/WebAuthn) | Authentication | An authentication mechanism where cryptographic keys are bound to the legitimate domain, making it fundamentally impossible to phish — even if the victim visits a cloned page, the key will not authenticate on the wrong origin. | Day 09 |
